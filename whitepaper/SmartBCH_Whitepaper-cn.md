...
一个epoch持续2016个块（大约两周时间）。在一个epoch内，BCH的持币人通过施加时间锁的UTXO来证明自己的拥有权，并且使用其拥有的数额来为验证者投票；而矿池则使用coinbase交易来投票。这是一种混合的共识模型：算力和币混合投票。投票的过程在比特币现金的主链上进行，并且是无许可的：成为新的验证者只需要矿工和（或）持币人的支持。
