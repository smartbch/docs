...
Moeing链对比特币现金的贡献是什么呢？它为新的功能提供了试验场：它拥有更短的确认时间，它不但支持EVM和Web3，而且支持得现有方案更好（通量更高）。它提供了一个新的渠道，邀请更多的开发者和用户加入到比特币现金的生态系统当中来。
...
1. 共识引擎和和交易执行引擎可以同时运行
2. 不同的交易可以同时执行

为了让共识引擎和和交易执行引擎同时运行，MoeingEVM采用了这样的策略：当一个新的区块被提交时，其内部的交易并没有被立即执行；它们只是被存储了起来，作为世界状态的一部分；之后世界状态的Merkle Root被计算出来让下一个区块可以被打包和投票，与此同时，之前被存储的交易会被检视和执行。
...
MoeingDB是一个领域定制的数据库，专门用来存储区块链的历史，它专门针对上面的负载特征而开发。基于它的能力，可以开发处一个完全开源的、高QPS的Web3 API，这对于Moeing链和以太坊都有正面意义。我们希望它可以使得Web3 API供应市场变得更加去中心化。
